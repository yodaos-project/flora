...
public:
	...
	int32_t recv_get(const char* name, shared_ptr<Caps>& msg,
		...
		capsh->caps = msg;
		int32_t r = env->CallIntMethod(javaThis,
				g_constants.java_methods[JAVA_METHOD_GET_CALLBACK],
				nameobj, (jlong)capsh, (jlong)replyh);
		reply = replyh->caps;
		delete replyh;
		env->PopLocalFrame(0);
		detachJavaThread();
...
static void com_rokid_flora_Client_native_init(JNIEnv *env, jclass thiz) {
	...
	g_constants.java_fields[JAVA_FIELD_REPLY_MSG] =
		env->GetFieldID(g_constants.java_reply_cls, "msg", "Lcom/rokid/flora/Caps;");
	g_constants.java_fields[JAVA_FIELD_REPLY_EXTRA] =
		env->GetFieldID(g_constants.java_reply_cls, "extra", "Ljava/lang/String;");
...
static jint com_rokid_flora_Client_native_connect(JNIEnv *env,
		jobject thiz, jstring uri, jint bufsize) {
	...
	uristr[slen] = '\0';
	int32_t r = flora::Client::connect(uristr, &handle->callback, bufsize,
				handle->cli);
	delete uristr;
...
static void com_rokid_flora_Client_native_destroy(JNIEnv *env,
...
static jint com_rokid_flora_Client_native_subscribe(JNIEnv *env,
		jobject thiz, jlong handle, jstring name, jint type) {
	NativeHandle* nh = (NativeHandle*)handle;
	char* namestr;
	jsize slen;
...
static jint com_rokid_flora_Client_native_unsubscribe(JNIEnv *env,
		jobject thiz, jlong handle, jstring name, jint type) {
	NativeHandle* nh = (NativeHandle*)handle;
	char* namestr;
	jsize slen;
...
static jint com_rokid_flora_Client_native_post(JNIEnv *env,
		jobject thiz, jlong handle, jstring name, jlong msg, jint type) {
	NativeHandle* nh = (NativeHandle*)handle;
	char* namestr;
	jsize slen;
	slen = env->GetStringUTFLength(name);
...
static void gen_java_reply_list(JNIEnv* env, jobject replys,
	...
	for (it = nreps.begin(); it != nreps.end(); ++it) {
		...
		env->SetIntField(jr, g_constants.java_fields[JAVA_FIELD_REPLY_RETCODE], (*it).ret_code);
		...
		env->SetObjectField(jr, g_constants.java_fields[JAVA_FIELD_REPLY_EXTRA], je);
		env->CallVoidMethod(replys, g_constants.java_methods[JAVA_METHOD_LIST_ADD],
				jr);
	}
}

static jint com_rokid_flora_Client_native_get(JNIEnv *env,
		jobject thiz, jlong handle, jstring name, jlong msg,
		jobject replys, jint timeout) {
	NativeHandle* nh = (NativeHandle*)handle;
	char* namestr;
	jsize slen;
	slen = env->GetStringUTFLength(name);
	...
	vector<flora::Reply> nreps;
	shared_ptr<Caps> t;
	int32_t r = nh->cli->get(namestr, cmsg ? cmsg->caps : t,
			nreps, timeout);
	delete namestr;
	if (r == FLORA_CLI_SUCCESS) {
...
static void com_rokid_flora_Client_native_set_pointer(JNIEnv *env,
		jobject thiz, jlong src, jlong dst) {
...
static JNINativeMethod _cli_nmethods[] = {
	...
	{ "native_destroy", "(J)V",
		(void*)com_rokid_flora_Client_native_destroy },
