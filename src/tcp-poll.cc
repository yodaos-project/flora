...
void TCPPoll::new_adapter(int fd) {
	shared_ptr<TCPAdapter> adap = make_shared<TCPAdapter>(fd, max_msg_size);
...
bool TCPPoll::read_from_client(shared_ptr<TCPAdapter>& adap) {
	int32_t r = adap->read();
	if (r != TCP_ADAPTER_SUCCESS)
		return false;
